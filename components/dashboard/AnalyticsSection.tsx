...
export default function AnalyticsSection({ userPlan }: { userPlan: PlanName }) {
  const [period, setPeriod] = useState("7j");
  const [quotaData, setQuotaData] = useState({ daily: 0, monthly: 0, pct: 0, remaining: 0 });
  const planCfg = PLANS_CONFIG[userPlan] || PLANS_CONFIG.SPARK;
  ...
  return (
    <div style={{ padding: "24px", overflowY: "auto", height: "100%" }}>
      ...
      <div style={{ background: `linear-gradient(135deg,${planCfg.color}12,rgba(0,0,0,.4))`, border: `1px solid ${planCfg.color}40`, borderRadius: 16, padding: "18px 20px", marginBottom: 18, display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
        <div>
          <div style={{ fontSize: 11, color: planCfg.color, fontWeight: 800, letterSpacing: ".1em", marginBottom: 4 }}>PLAN {planCfg.label} — QUOTA AUJOURD&apos;HUI</div>
          <div style={{ fontSize: 28, fontWeight: 900, color: "#fff" }}>{quotaData.daily}<span style={{ fontSize: 14, color: "rgba(255,255,255,.4)", fontWeight: 400 }}>/{planCfg.daily === 999 ? "∞" : planCfg.daily} req</span></div>
        </div>
        <div style={{ flex: 1, minWidth: 140 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
            <span style={{ fontSize: 11, color: "rgba(255,255,255,.4)" }}>{quotaData.pct.toFixed(0)}% utilisé</span>
            <span style={{ fontSize: 11, color: quotaData.remaining > 0 ? "#39ff14" : "#ff3366", fontWeight: 700 }}>{planCfg.daily === 999 ? "Illimité" : `${quotaData.remaining} restantes`}</span>
          </div>
          <div style={{ height: 8, background: "rgba(255,255,255,.08)", borderRadius: 4, overflow: "hidden" }}>
            <div style={{ height: "100%", width: `${quotaData.pct}%`, background: quotaData.pct >= 90 ? "linear-gradient(90deg,#ffd700,#ff3366)" : `linear-gradient(90deg,${planCfg.color},#6366f1)`, borderRadius: 4, transition: "width .5s ease" }} />
          </div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(170px,1fr))", gap: 12, marginBottom: 20 }}>
        {[{ label: "Requêtes aujourd'hui", value: quotaData.daily, icon: "⚡", color: "#00d4ff" }, { label: "Total ce mois", value: quotaData.monthly, icon: "◎", color: "#ffd700" }, { label: "Tokens max/requête", value: planCfg.maxTokens, icon: "🔢", color: "#39ff14" }, { label: "Limite journalière", value: planCfg.daily === 999 ? "∞" : planCfg.daily, icon: "⏱", color: "#ff6b35" }].map(s => (
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 4, height: 110 }}>
          {data.map((v, i) => (
            <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
              <div style={{ width: "100%", background: `linear-gradient(180deg,${ACCENT},#6366f1)`, borderRadius: "4px 4px 0 0", height: `${(v / max) * 100}%`, transition: "height .4s ease", minHeight: 4, boxShadow: `0 0 6px ${ACCENT}40` }} />
