...
const notifs = [
  ...
  { icon: "💬", text: "Essaie le Multi-Chat IA avec Claude Sonnet 4", time: "Commencer →", color: "#cc785c" },
...
export default function Topbar({ userName, userPlan }: TopbarProps) {
  ...
  useEffect(() => {
    fetch("/api/usage")
      .then(r => r.json())
      .then(d => setQuotaData({ daily: d.daily || 0, pct: d.pct || 0, limit: d.limits?.daily || planCfg.daily }))
      .catch(() => {});
  }, [planCfg.daily]);
    ...
    <div style={{ height: 56, borderBottom: `1px solid ${BORDER}`, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px", background: "rgba(255,255,255,.01)", flexShrink: 0 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 11, color: planCfg.color, fontWeight: 700 }}>{userPlan}</span>
          <div style={{ width: 80, height: 5, background: "rgba(255,255,255,.1)", borderRadius: 3, overflow: "hidden" }}>
            <div style={{ height: "100%", width: `${quotaData.pct}%`, background: quotaData.pct >= 80 ? "linear-gradient(90deg,#ffd700,#ff3366)" : "linear-gradient(90deg,#00d4ff,#6366f1)", borderRadius: 3, transition: "width .4s" }} />
          </div>
          <span style={{ fontSize: 11, color: quotaData.pct >= 80 ? "#ffd700" : ACCENT, fontWeight: 700 }}>
            {quotaData.daily}/{planCfg.daily === 999 ? "∞" : planCfg.daily}
          </span>
        </div>
