  ...
  return (
    <section style={{ padding: "80px 40px", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 48, position: "relative", zIndex: 2 }}>
        <p style={{ fontSize: 11, letterSpacing: ".25em", color: "#ff3366", fontWeight: 800, marginBottom: 10, textTransform: "uppercase" }}>Interface dashboard</p>
        <h2 style={{ fontSize: "clamp(24px,4vw,48px)", fontWeight: 800 }}>Une interface qui<br /><span style={{ color: "#ffd700" }}>redéfinit la productivité.</span></h2>
      </div>
      <Card3D intensity={6} style={{ maxWidth: 960, margin: "0 auto", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 24, overflow: "hidden", boxShadow: "0 40px 100px rgba(0,0,0,0.6), 0 0 80px rgba(0,212,255,0.08)", position: "relative", zIndex: 2 }}>
        {/* Fake browser bar */}
        ...
        <div style={{ display: "flex", height: 340 }}>
          <div style={{ width: 56, background: "rgba(255,255,255,0.02)", borderRight: `1px solid ${BORDER}`, display: "flex", flexDirection: "column", alignItems: "center", padding: "16px 0", gap: 14 }}>
            <div style={{ width: 32, height: 32, borderRadius: 9, background: "linear-gradient(135deg,#00d4ff,#6366f1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13 }}>⬡</div>
            {["💬", "⚙️", "🤖", "🎬", "📊"].map((ic, i) => <div key={i} style={{ width: 32, height: 32, borderRadius: 8, background: i === 0 ? "rgba(0,212,255,0.15)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>{ic}</div>)}
          </div>
          <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "16px 20px", gap: 10 }}>
            <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
              <div style={{ width: 28, height: 28, borderRadius: "50%", background: "rgba(204,120,92,0.2)", border: "1px solid #cc785c50", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, color: "#cc785c", flexShrink: 0 }}>⬡</div>
              <div style={{ padding: "10px 14px", background: "rgba(255,255,255,0.04)", border: `1px solid ${BORDER}`, borderRadius: "4px 14px 14px 14px", fontSize: 12, color: "rgba(255,255,255,0.7)", maxWidth: 420, lineHeight: 1.6 }}>
                Bonjour ! Je suis <strong style={{ color: "#fff" }}>AKASHA AI</strong>, ton assistant IA ultime avec accès à 47 outils. Comment puis-je t&apos;aider aujourd&apos;hui ? ✨
              </div>
            </div>
            <div style={{ display: "flex", gap: 10, alignItems: "flex-start", justifyContent: "flex-end" }}>
              <div style={{ padding: "10px 14px", background: "rgba(0,212,255,0.1)", border: "1px solid rgba(0,212,255,0.2)", borderRadius: "14px 4px 14px 14px", fontSize: 12, color: "rgba(255,255,255,0.8)", maxWidth: 300, lineHeight: 1.6 }}>
                Génère une landing page complète pour ma startup SaaS en React
              </div>
              <div style={{ width: 28, height: 28, borderRadius: "50%", background: "linear-gradient(135deg,#00d4ff,#6366f1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, flexShrink: 0 }}>T</div>
            </div>
            <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
              <div style={{ width: 28, height: 28, borderRadius: "50%", background: "rgba(204,120,92,0.2)", border: "1px solid #cc785c50", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, color: "#cc785c", flexShrink: 0 }}>⬡</div>
              <div style={{ padding: "10px 14px", background: "rgba(255,255,255,0.04)", border: `1px solid ${BORDER}`, borderRadius: "4px 14px 14px 14px", fontSize: 12, color: "rgba(255,255,255,0.7)", lineHeight: 1.6 }}>
                Voici ta landing page complète avec hero, features, pricing et FAQ… <span style={{ color: ACCENT, animation: "pulse 1s infinite" }}>▌</span>
              </div>
            </div>
          </div>
