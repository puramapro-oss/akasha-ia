...
export async function GET() {
  try {
    ...
    const remaining = plan.daily - daily;
    const pct = Math.min((daily / plan.daily) * 100, 100);

    return Response.json({ daily, monthly, tokens, remaining, pct, blocked: remaining <= 0, plan: planName, limits: { daily: plan.daily, monthly: plan.monthly, maxTokens: plan.maxTokens } });
  } catch {
